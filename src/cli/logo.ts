...
/**
 * A decorative splash is only appropriate in an interactive terminal — never
 * in CI, nor when output is piped/redirected (there it would be noise or would
 * corrupt captured output). Mirrors the TTY gating the prompts already use.
 */
export function shouldShowSplash(io: CliIO): boolean {
  return io.interactive === true && !io.env.CI;
}
